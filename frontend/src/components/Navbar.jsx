...
import { Menu, X, Home, Users, Settings, User, LogOut, ChevronDown, HeartHandshake } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Rooms', path: '/rooms', icon: <BedDouble className="w-4 h-4 mr-2" /> },
    { name: 'Find Roommates', path: '/find-roommates', icon: <Users className="w-4 h-4 mr-2" /> },
    { name: 'Preferences', path: '/preferences', icon: <Settings className="w-4 h-4 mr-2" /> },
  ];
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex-shrink-0 flex items-center">
            <NavLink to="/" className="flex items-center gap-2.5 group">
              ...
              <span className="font-bold text-xl sm:text-2xl tracking-tight text-[#4A3B32]">RoomSync</span>
            </NavLink>
          </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <div className="flex items-center text-sm font-medium text-[#4A3B32]">
                  <span className="hidden lg:block mr-2"> {fullname} </span>
                  <ChevronDown className={`w-4 h-4 text-[#8B7E74] transition-transform duration-300 ${isProfileDropdownOpen ? 'rotate-180' : ''}`} />
                </div>
              </button>
              ...
              <div
                className={`absolute right-0 mt-3 w-56 rounded-2xl shadow-xl shadow-[#4A3B32]/10 bg-[#FDFAF6] ring-1 ring-[#4A3B32]/5 origin-top-right transition-all duration-300 ease-out ${
                  isProfileDropdownOpen ? 'transform opacity-100 scale-100' : 'transform opacity-0 scale-95 pointer-events-none'
                }`}
              >
                <div className="p-2">
                  <div className="px-3 py-3 border-b border-[#8B7E74]/15 mb-2">
                   <p className="text-sm font-semibold text-[#4A3B32]"> {fullname}</p>
                    <p className="text-xs text-[#8B7E74] truncate mt-0.5"> {email}</p>
                  </div>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center w-full px-3 py-2.5 text-sm rounded-xl transition-colors duration-200 ${
                        isActive ? 'bg-[#F5F2E9] text-[#C27A62] font-medium' : 'text-[#4A3B32] hover:bg-[#F5F2E9]'
                      }`
                    }
                  >
        ...
        <div className="px-4 pt-4 pb-6 space-y-2">
          ...
          <div className="border-t border-[#8B7E74]/15 my-4 pt-4">
            <div className="flex items-center px-4 mb-4">
              <div className="w-11 h-11 rounded-full bg-[#C27A62] flex items-center justify-center text-[#FDFAF6] font-semibold text-base shadow-sm">
                {profilePicture ? (
                    <img
                        src={profilePicture}
                        alt={fullname}
              ...
              <div className="ml-3">
                <p className="text-base font-semibold text-[#4A3B32]">{fullname}</p>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center w-full px-4 py-3 rounded-2xl text-base font-medium transition-colors duration-200 ${
                  isActive ? 'bg-[#F5F2E9] text-[#C27A62]' : 'text-[#4A3B32] hover:bg-[#F5F2E9]'
                }`
              }
            >
